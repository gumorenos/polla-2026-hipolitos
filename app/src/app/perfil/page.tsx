...
import { MOCK_USER, MOCK_STANDINGS } from '../../lib/mockData';
...
export default function PerfilPage() {
  ...
  // Fetch statistics from mock standings
  const userStats = MOCK_STANDINGS.find(s => s.userId === MOCK_USER.id) ?? {
    points: 10,
    exacts: 1,
    tendencies: 1,
    consolations: 2,
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="lg:col-span-4 card-base p-5 space-y-4">
            <div className="flex flex-col items-center text-center gap-2 pb-4 border-b border-border-subtle">
              ...
              <div>
                <h3 className="font-bold text-text-primary text-base">{MOCK_USER.name}</h3>
                <span className="text-xs text-text-secondary">@{MOCK_USER.displayName}</span>
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                ...
                <div className="space-y-1 opacity-60">
                  ...
                  <div className="relative">
                    ...
                    <input
                      type="text"
                      value={new Date(MOCK_USER.createdAt).toLocaleDateString('es-ES')}
                      className="field pl-10 bg-bg-primary cursor-not-allowed"
                      disabled
                    />
                  </div>
